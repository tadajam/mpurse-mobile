...
export class InAppBrowserService {
  private inAppBrowserTabs: AppInfo[] = [];
  ...
  open(target: string): void {
    const inAppBrowser = this.inAppBrowser.create(
    ...
    this.inAppBrowserTabs = [
      {
        inAppBrowserObject: inAppBrowser,
        href: '',
        origin: '',
        title: '',
        icon: '',
    ...
    this.inAppBrowserObjectSubject.next(inAppBrowser);
  ...
  getTabsLength(): number {
    return this.inAppBrowserTabs.length;
  }

  getTabs(): AppInfo[] {
    if (this.inAppBrowserTabs.length !== 0) {
      return this.inAppBrowserTabs;
    } else {
      return [];
    }
  }
  ...
  generateUrl(target: string): string {
    try {
      ...
      switch (this.preferenceService.getSearchEngine()) {
        case 'google':
          return 'https://www.google.com/search?q=' + target;
        case 'duckduckgo':
          return 'https://duckduckgo.com/?q=' + target;
      }
    }
  }
