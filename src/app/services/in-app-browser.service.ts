...
export class InAppBrowserService {
  ...
  lastBrowse: { href: string; title: string; icon: string } = null;
  favorites: { href: string; title: string; icon: string }[] = [];
  histories: { href: string; title: string; icon: string }[] = [];

  constructor(private zone: NgZone, private inAppBrowser: InAppBrowser) {}
  ...
  open(target: string): void {
    ...
    this.inAppBrowserObject.on('message').subscribe({
      next: (event: InAppBrowserEvent) => {
        if (event['data']['action'] === InPageMessage.InitRequest) {
          this.lastBrowse = {
  ...
  addFavorite(favorite: { href: string; title: string; icon: string }): void {
    if (favorite) {
  ...
  addHistory(history: { href: string; title: string; icon: string }): void {
    if (history) {
